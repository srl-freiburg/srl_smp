...
::find_nearest_vertex_ball (state_t *state_in,vertex_t **data_out,double radius){
    ...
    //        int k=10;
      ...
      for (typename list<void*>::iterator iter = list_vertices_in_ball.begin(); iter != list_vertices_in_ball.end(); iter++) {
        ...
        exact_connection = -1;
        this->extender.dt_=this->RES_POSQ;



        if(LEARN>0){
          /// set the proper distance metric
          
          this->cost_evaluator.set_learn(LEARN);
          ...
          /// set the distance metric back to the POSQ extender
          this->cost_evaluator.set_learn(0);
...
::iteration () {
  ...
  this->extender.dt_=this->DT;
  this->extender.rho_endcondition=this->RHO_END;
